...
const AllProjects = () => {
  ...
  useEffect(() => {
    const getNews = async () => {
      const { data } = await axios.get(`http://server.wathincompanyltd.com/api/auth-project?author=${user.name}`, {
        headers: {
  ...
  const handleDelete = async (id) => {
    const response = await axios.delete(`http://server.wathincompanyltd.com/api/project/${id}`, {
      headers: {
        Authorization: `Bearer ${localStorage.getItem('token')}`
      }
    })
